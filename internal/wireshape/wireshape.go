...
// SnapshotHeaderFirstLine opens every file scripts/snapshot_openapi_schemas.py
// writes; the file's second line names the platform commit it was derived
// from.
const SnapshotHeaderFirstLine = "# GENERATED by scripts/snapshot_openapi_schemas.py - do not edit."

var snapshotSourceRE = regexp.MustCompile(`^# Source: \S+ at platform commit ([0-9a-f]{40})$`)

// SnapshotSourceCommit returns the platform commit a generated snapshot in
// specDir was derived from, or "" when no *.yaml there carries the
// generated header (a checkout of the platform's docs/api). It fails when
// only some files carry the header, when a header does not name a full
// 40-character commit, or when the files name different commits: a partly
// regenerated snapshot has no single revision to pin.
func SnapshotSourceCommit(specDir string) (string, error) {
	...
	for _, e := range entries {
		...
		data, readErr := os.ReadFile(filepath.Join(specDir, e.Name()))
		if readErr != nil {
			return "", fmt.Errorf("read %s: %w", e.Name(), readErr)
		}
		lines := strings.SplitN(string(data), "\n", 3)
		if lines[0] != SnapshotHeaderFirstLine {
			plain = append(plain, e.Name())
			continue
		}
		var m []string
		if len(lines) > 1 {
			m = snapshotSourceRE.FindStringSubmatch(lines[1])
		}
		if m == nil {
			return "", fmt.Errorf("%s: the generated header does not name the full 40-character platform commit the file was derived from", e.Name())
		}
		commits[e.Name()] = m[1]
	}
	if len(commits) == 0 {
	...
	if len(plain) > 0 {
		sort.Strings(plain)
		return "", fmt.Errorf("%s mixes generated snapshot files with plain specs (%s); regenerate the whole snapshot", specDir, strings.Join(plain, ", "))
	}
	...
	sort.Strings(names)
	pinned := commits[names[0]]
	for _, n := range names[1:] {
		if commits[n] != pinned {
			parts := make([]string, 0, len(names))
			for _, m := range names {
				parts = append(parts, m+" at "+commits[m][:9])
			}
			return "", fmt.Errorf("%s holds snapshot files from different platform commits (%s); regenerate the whole snapshot at one commit", specDir, strings.Join(parts, ", "))
		}
	}
	return pinned, nil
}
