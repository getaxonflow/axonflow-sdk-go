...
- **MCP Policy-Check Endpoints**: Standalone policy validation for external orchestrators (LangGraph, CrewAI)
  - `MCPCheckInput(ctx, req)`: Validate MCP queries/commands against input policies (SQLi, PII, dynamic) before execution
  - `MCPCheckOutput(ctx, req)`: Validate MCP response data against output policies (PII redaction, exfiltration limits) after execution
