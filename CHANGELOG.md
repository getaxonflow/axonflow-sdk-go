...
- **Typed policy authoring (axonflow-enterprise#3746).** A v11.0.0 platform authors policy as a
  ...
  `*TypedPolicyRefusal` carrying the status, the platform's reason, the `Policy` a tier refusal
  names, any findings and `Retry-After`. The typed answers carry every member the platform
  sends: `TypedAuthoringEdition`'s `CatalogDigest`, `RegistryVersion` and `CatalogFixture`; a
  `TemplateOmissionReport` (`TemplateOmissions` and `TemplateOmissionsUnavailable` on the
  publication and the activation), naming the organization template's controls a document
...
- **Examples for the v11.0.0 platform.** `examples/pep_handshake` declares an enforcement
  ...
  `examples/typed_policies` authors policy as a typed document (it publishes and activates only
  when asked, prints the publication's template-omission report before it activates, and
  embeds its default document, so it runs from any directory). Both exit non-zero when a step
  fails, including a publication or activation the example asked for and the platform refused,
  and CI builds them. The examples README runs the handshake example first, and the README
  gains a "v11.0.0 platform" section naming what each v11 surface needs from the platform.
...
- **`runtime-e2e/x_axonflow_client/main.go` proves the release against the LIVE platform.** A bump changes one
  product fact - the version in version.go - and every governed request carries it as
  `X-Axonflow-Client: sdk-go/<version>` (ADR-050 §4). The leg reads
